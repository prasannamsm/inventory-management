    ...
    <div v-else>
          ...
          <table class="reports-table">
            ...
            <tbody>
              <tr v-for="q in quarterlyData" :key="q.quarter">
                ...
                <td>{{ formatCurrencyWithDecimals(q.total_revenue, selectedCurrency, 2) }}</td>
            ...
            <div v-for="month in monthlyData" :key="month.month" class="bar-wrapper">
              <div class="bar-container">
                <div
                  ...
                  :title="formatCurrencyWithDecimals(month.revenue, selectedCurrency, 2)"
                ></div>
              </div>
          ...
          <table class="reports-table">
            ...
            <tbody>
              <tr v-for="(month, index) in monthlyData" :key="month.month">
                ...
                <td>{{ formatCurrencyWithDecimals(month.revenue, selectedCurrency, 2) }}</td>
                <td>
      ...
      <div class="stats-grid">
        <div class="stat-card">
          ...
          <div class="stat-value">{{ formatCurrencyWithDecimals(totalRevenue, selectedCurrency, 2) }}</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">{{ t('reports.summary.avgMonthly') }}</div>
          <div class="stat-value">{{ formatCurrencyWithDecimals(avgMonthlyRevenue, selectedCurrency, 2) }}</div>
        </div>
        <div class="stat-card">
...
export default {
  ...
  setup() {
    ...
    // Computed summary stats — derived reactively, no manual loops
    ...
    // Pre-computed max revenue so getBarHeight doesn't re-scan on every bar render
    ...
    const loadData = async () => {
      try {
        loading.value = true
        error.value = null
        // Reports show full-year trends; pass warehouse/category/status but not month
    ...
    // Month key lookup for i18n — avoids hardcoded English month names
    ...
    const getChangeValue = (current, previous) => {
      ...
      if (change > 0) return '+' + formatCurrencyWithDecimals(change, selectedCurrency.value, 2)
      if (change < 0) return '-' + formatCurrencyWithDecimals(Math.abs(change), selectedCurrency.value, 2)
      return formatCurrencyWithDecimals(0, selectedCurrency.value, 2)
    }
